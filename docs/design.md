...
VR の背景を **不透明な背景色** か **パススルーカメラの映像** から選べる。
UI はコントロールパネルの「VR背景」セレクタ。

パススルーは WebXR では **`immersive-ar` セッション**として提供される
（`immersive-vr` に「パススルーを有効にする」ようなオプションは無い）。
したがってモードの選択は、要求するセッション種別の切り替えになる。

| 選択 | セッション | 背景 |
|---|---|---|
| 背景色 | `immersive-vr` | 選んだ背景色で塗りつぶす |
| パススルー | `immersive-ar` | キャンバスを透過させ、外側の映像を合成させる |
...
- **描画バッファの `alpha` は `getContext` の時点で決まる。** 既存のコンテキストを
  ...
  として明示する。
- パススルー中は `scene.background = null` と `renderer.setClearAlpha(0)` にする。
  不透明な背景色を残すとパススルー映像を覆ってしまう。VR 終了時に元へ戻す。
- 対応判定は `isSessionSupported('immersive-vr')` と `('immersive-ar')` を**別々に**行う。
  VR に対応していてもパススルーに対応しない端末があるため、非対応なら選択肢を
  無効化して理由を表示する。
...
### 11.6 VR 入場時のモデル配置（`placement.ts`）
...
- XR 対応状況に応じた UI の分岐（`navigator.xr` を差し替えて検証する。
  実機が無くても、VR ボタンと「VR背景」セレクタの表示・パススルー選択肢の
  有効無効・非対応時の注記まで確認できる）
